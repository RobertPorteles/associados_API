Aqui está a documentação organizada em um Markdown profissional, limpo e fácil de consultar:📑 Documentação Técnica da API — Sistema de AssociadosDocumentação dos endpoints, regras de negócio e padrões de comunicação da API.🔐 Autenticação e CabeçalhosTodos os requests autenticados devem obrigatoriamente enviar os headers abaixo:HeaderValorAuthorizationBearer {{seu_token_jwt}}Content-Typeapplication/json🏗️ 1. Edição do Endereço ComercialAtualiza os dados de uma empresa. O sistema permite enviar apenas os dados básicos ou incluir o endereço para criação/atualização simultânea.A. Apenas dados da empresaPUT /api/v1/empresas/{idEmpresa}JSON{
  "idAssociado": 1,
